...
import {
  getApiTokens,
  deleteApiToken,
} from "@/app/builder/_actions/settings-actions"; // Import Actions
...
export default async function ApiIntegrationPage({ params }: Props) {
  const { projectId } = await params;

  // 1. Ambil data asli dari Database
  const { data: tokens } = await getApiTokens(projectId);

  // Wrapper untuk delete action
  async function handleDelete(id: string) {
    "use server";
    await deleteApiToken(projectId, id);
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8 min-h-screen bg-white dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link
          href={`/builder/${projectId}/settings/api-integration/new`}>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-[#3B82F6] text-white">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-left w-[30%]">
                  Description
                </th>
                <th className="px-6 py-4 font-semibold text-center w-[15%]">
                  Created
                </th>
                <th className="px-6 py-4 font-semibold text-center w-[15%]">
                  Expires
                </th>
                <th className="px-6 py-4 font-semibold text-center w-[10%]">
                  Action
                </th>
              </tr>
            </thead>
