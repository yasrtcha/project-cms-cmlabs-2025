...
export default async function ActivityLogsPage({ params }: Props) {
  ...
  const { data: logs } = await getProjectActivityLogs(projectId);
          ...
          {!logs || logs.length === 0 ? (
            ...
            logs.map((log: any) => {
              ...
              return (
                <div
                  key={log.id}
                  className="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                  {/* 1. Actor (User) */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-600 dark:text-slate-300">
                      <span className="text-gray-400 dark:text-slate-500 mr-1">
                        on {log.entityType}:
                      </span>
