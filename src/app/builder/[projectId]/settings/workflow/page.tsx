...
import {
  getWorkflows,
  deleteWorkflow,
} from "@/app/builder/_actions/settings-actions"; // Import deleteWorkflow
import {
  Plus,
  Pencil,
  GitPullRequest,
...
export default async function WorkflowPage({ params }: Props) {
  ...
  const { data: workflows } = await getWorkflows(projectId);
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8 min-h-screen bg-white dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link href={`/builder/${projectId}/settings/workflow/new`}>
                ...
                workflows.map((workflow: any) => (
                  <tr
                    ...
                    <td className="px-6 py-4 text-gray-600 dark:text-slate-300 align-middle">
                      ...
                      {workflow.appliedContentTypes &&
                      workflow.appliedContentTypes.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {workflow.appliedContentTypes.map((ct: any) => (
                            <span
                              key={ct.name}
                              className="px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 text-xs font-medium">
                              {ct.name}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="px-6 py-4 align-middle">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <button className="p-1.5 rounded-md bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 transition-colors">
                          <Pencil size={16} />
                        </button>
                      </div>
                    </td>
